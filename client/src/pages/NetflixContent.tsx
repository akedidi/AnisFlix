...
export default function NetflixContent() {
  ...
  const { restoreScrollPosition } = useScrollPosition('netflix-content'    </div>
  );
}
  ...
  const { data: moviesData, isLoading: moviesLoading } = useMoviesByProvider(8, currentPage    </div>
  );
}
  const { data: seriesData, isLoading: seriesLoading } = useSeriesByProvider(8, currentPage    </div>
  );
}
      ...
      <div className="relative bg-gradient-to-b from-primary/20 to-background">
        <div className="container mx-auto px-4 md:px-8 lg:px-12 py-8">
          <div className="flex items-center gap-4 mb-4">
            ...
            />
            <div>
              <h1 className="text-3xl md:text-4xl font-bold">Netflix</h1>
              <p className="text-muted-foreground">
                Découvrez tous les films et séries disponibles sur Netflix.
              </p>
            </div>
          </div>
        </div>
