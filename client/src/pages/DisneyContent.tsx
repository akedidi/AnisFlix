...
export default function DisneyContent() {
  ...
  const { restoreScrollPosition } = useScrollPosition('disney-content'    </div>
  );
}
  ...
  const { data: moviesData, isLoading: moviesLoading } = useMoviesByProvider(337, currentPage    </div>
  );
}
  const { data: seriesData, isLoading: seriesLoading } = useSeriesByProvider(337, currentPage    </div>
  );
}
