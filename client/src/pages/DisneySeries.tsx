...
export default function DisneySeries() {
  ...
  const { restoreScrollPosition } = useScrollPosition('disney-series');

  // Fetch data from TMDB - Only Disney+ series
  const { data: seriesData, isLoading: seriesLoading } = useSeriesByProvider(337, currentPage);
  const { data: searchResults = [] } = useMultiSearch(searchQuery);

  // Disney+ specific series genres
  const { data: actionSeriesData } = useSeriesByProviderAndGenre(337, 10759); // Action & Adventure
  const { data: comedySeriesData } = useSeriesByProviderAndGenre(337, 35); // Comedy
  const { data: dramaSeriesData } = useSeriesByProviderAndGenre(337, 18); // Drama
  const { data: crimeSeriesData } = useSeriesByProviderAndGenre(337, 80); // Crime
  const { data: mysterySeriesData } = useSeriesByProviderAndGenre(337, 9648); // Mystery
  const { data: sciFiSeriesData } = useSeriesByProviderAndGenre(337, 10765); // Sci-Fi & Fantasy
  const { data: thrillerSeriesData } = useSeriesByProviderAndGenre(337, 53); // Thriller
  const { data: animationSeriesData } = useSeriesByProviderAndGenre(337, 16); // Animation
  const { data: documentarySeriesData } = useSeriesByProviderAndGenre(337, 99); // Documentary
  const { data: realitySeriesData } = useSeriesByProviderAndGenre(337, 10764); // Reality
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl font-semibold">Séries Disney+</h1>
            </div>
            ...
            <div className="flex items-center gap-2">
              <SearchBar
                ...
                placeholder="Rechercher des séries Disney+..."
              />
              <LanguageSelect />
              <ThemeToggle />
            </div>
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        {!searchQuery && (
          <>
            {/* Latest Disney+ Series */}
            <div className="space-y-6">
              <h2 className="text-2xl font-semibold">Dernières séries Disney+</h2>
              {seriesLoading ? (
                <div className="text-center py-12">
                  <p className="text-muted-foreground">Chargement des séries Disney+...</p>
                </div>
              ) : series.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {familySeries.length > 0 && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
                  {familySeries.slice(0, 10).map((serie: TransformedMedia) => (
                    <div key={serie.id} className="w-full">
                      <MediaCard
                        {...serie}
                        mediaType="tv"
            ...
            {actionSeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Action Disney+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {comedySeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Comédie Disney+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {dramaSeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Drame Disney+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {crimeSeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Crime Disney+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {mysterySeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Mystère Disney+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {sciFiSeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Science-Fiction Disney+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {thrillerSeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Thriller Disney+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {realitySeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Téléréalité Disney+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
