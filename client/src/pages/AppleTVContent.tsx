...
export default function AppleTVContent() {
  ...
  const { restoreScrollPosition } = useScrollPosition('apple-tv-content'    </div>
  );
}
  ...
  const { data: moviesData, isLoading: moviesLoading } = useMoviesByProvider(350, currentPage    </div>
  );
}
  const { data: seriesData, isLoading: seriesLoading } = useSeriesByProvider(350, currentPage    </div>
  );
}
