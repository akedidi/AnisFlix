...
export default function HBOMaxMovies() {
  const { t } = useLanguage();
  const [searchQuery, setSearchQuery] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const { restoreScrollPosition } = useScrollPosition('hbo-max-movies');

  // Fetch data from TMDB - Only HBO Max movies
  const { data: moviesData, isLoading: moviesLoading } = useMoviesByProvider(1899, currentPage);
  const { data: searchResults = [] } = useMultiSearch(searchQuery);

  // HBO Max specific movie genres
  const { data: actionMoviesData } = useMoviesByProviderAndGenre(1899, 28); // Action
  const { data: comedyMoviesData } = useMoviesByProviderAndGenre(1899, 35); // Comedy
  const { data: dramaMoviesData } = useMoviesByProviderAndGenre(1899, 18); // Drama
  const { data: romanceMoviesData } = useMoviesByProviderAndGenre(1899, 10749); // Romance
  const { data: thrillerMoviesData } = useMoviesByProviderAndGenre(1899, 53); // Thriller
  const { data: horrorMoviesData } = useMoviesByProviderAndGenre(1899, 27); // Horror
  const { data: sciFiMoviesData } = useMoviesByProviderAndGenre(1899, 878); // Science Fiction
  const { data: fantasyMoviesData } = useMoviesByProviderAndGenre(1899, 14); // Fantasy
  const { data: animationMoviesData } = useMoviesByProviderAndGenre(1899, 16); // Animation
  const { data: documentaryMoviesData } = useMoviesByProviderAndGenre(1899, 99); // Documentary
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl font-semibold">Films HBO Max</h1>
            </div>
            ...
            <div className="flex items-center gap-2">
              <SearchBar
                ...
                placeholder="Rechercher des films HBO Max..."
              />
              <LanguageSelect />
              <ThemeToggle />
            </div>
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        {!searchQuery && (
          <>
            {/* Latest HBO Max Movies */}
            <div className="space-y-6">
              <h2 className="text-2xl font-semibold">Derniers films HBO Max</h2>
              {moviesLoading ? (
                <div className="text-center py-12">
                  <p className="text-muted-foreground">Chargement des films HBO Max...</p>
                </div>
              ) : movies.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
                ...
                <p className="text-muted-foreground">Aucun film HBO Max disponible</p>
              )}
            </div>
            ...
            {actionMovies.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Films Action HBO Max</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {comedyMovies.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Films Comédie HBO Max</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {romanceMovies.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Films Romance HBO Max</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {thrillerMovies.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Films Thriller HBO Max</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {horrorMovies.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Films Horreur HBO Max</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {sciFiMovies.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Films Science-Fiction HBO Max</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {fantasyMovies.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Films Fantastique HBO Max</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {animationMovies.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Films Animation HBO Max</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {documentaryMovies.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Documentaires HBO Max</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
