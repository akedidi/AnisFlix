...
export default function AppleTVSeries() {
  const { t } = useLanguage();
  const [searchQuery, setSearchQuery] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const { restoreScrollPosition } = useScrollPosition('apple-tv-series');

  // Fetch data from TMDB - Only Apple TV+ series
  const { data: seriesData, isLoading: seriesLoading } = useSeriesByProvider(350, currentPage);
  const { data: searchResults = [] } = useMultiSearch(searchQuery);

  // Apple TV+ specific series genres
  const { data: actionSeriesData } = useSeriesByProviderAndGenre(350, 10759); // Action & Adventure
  const { data: comedySeriesData } = useSeriesByProviderAndGenre(350, 35); // Comedy
  const { data: dramaSeriesData } = useSeriesByProviderAndGenre(350, 18); // Drama
  const { data: crimeSeriesData } = useSeriesByProviderAndGenre(350, 80); // Crime
  const { data: mysterySeriesData } = useSeriesByProviderAndGenre(350, 9648); // Mystery
  const { data: sciFiSeriesData } = useSeriesByProviderAndGenre(350, 10765); // Sci-Fi & Fantasy
  const { data: thrillerSeriesData } = useSeriesByProviderAndGenre(350, 53); // Thriller
  const { data: animationSeriesData } = useSeriesByProviderAndGenre(350, 16); // Animation
  const { data: documentarySeriesData } = useSeriesByProviderAndGenre(350, 99); // Documentary
  const { data: realitySeriesData } = useSeriesByProviderAndGenre(350, 10764); // Reality
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl font-semibold">Séries Apple TV+</h1>
            </div>
            ...
            <div className="flex items-center gap-2">
              <SearchBar
                ...
                placeholder="Rechercher des séries Apple TV+..."
              />
              <LanguageSelect />
              <ThemeToggle />
            </div>
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        {!searchQuery && (
          <>
            {/* Latest Apple TV+ Series */}
            <div className="space-y-6">
              <h2 className="text-2xl font-semibold">Dernières séries Apple TV+</h2>
              {seriesLoading ? (
                <div className="text-center py-12">
                  <p className="text-muted-foreground">Chargement des séries Apple TV+...</p>
                </div>
              ) : series.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {actionSeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Action Apple TV+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {comedySeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Comédie Apple TV+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {crimeSeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Crime Apple TV+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {mysterySeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Mystère Apple TV+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {sciFiSeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Science-Fiction Apple TV+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {thrillerSeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Thriller Apple TV+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {animationSeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Animation Apple TV+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
            ...
            {realitySeries.length > 0 && (
              <div className="space-y-6">
                <h2 className="text-2xl font-semibold">Séries Téléréalité Apple TV+</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-6">
