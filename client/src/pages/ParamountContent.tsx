...
export default function ParamountContent() {
  ...
  const { restoreScrollPosition } = useScrollPosition('paramount-content'    </div>
  );
}
  ...
  const { data: moviesData, isLoading: moviesLoading } = useMoviesByProvider(531, currentPage    </div>
  );
}
  const { data: seriesData, isLoading: seriesLoading } = useSeriesByProvider(531, currentPage    </div>
  );
}
