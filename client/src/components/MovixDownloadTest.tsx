...
export default function MovixDownloadTest() {
  ...
  const testMovieAPI = async () => {
    try {
      const response = await fetch(`https://api.movix.bet/api/films/download/${movieId}`);
  ...
  const testSeriesAPI = async () => {
    try {
      const response = await fetch(`https://api.movix.bet/api/series/download/${seriesId}/season/${season}/episode/${episode}`);
      const data = await response.json();
