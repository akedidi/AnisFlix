...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ==========================================
  // STREAM QUALITY HELPER FUNCTIONS
  // ==========================================
  ...
  app.get("/api/movix-proxy", async (req, res) => {
    try {
      ...
      let movixUrl;
      if (isAnimeRequest) {
        movixUrl = `https://api.movix.bet/${decodedPath}`;
      } else {
        movixUrl = `https://api.movix.bet/api/${decodedPath}`;
      }
  ...
  app.get("/api/darkibox", async (req, res) => {
    try {
      ...
      // Trier les sources par qualité (du meilleur au pire)
      sources.sort((a: any, b: any) => compareQuality(a.quality, b.quality));
