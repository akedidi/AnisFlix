...
struct LatestMoviesView: View {
    ...
    private func loadInitialData() async {
        ...
        print("🎬 Loading Latest Movies - Page 1")
        ...
        do {
            ...
            print("❌ Error loading latest movies: \(error)")
        }
    ...
    private func loadMoreData() {
        guard !isLoading && hasMore else { 
            print("⏸️ Load blocked - isLoading: \(isLoading), hasMore: \(hasMore)")
            return 
        }
        
        print("🔄 Starting load for page \(currentPage + 1)")
        isLoading = true
        ...
        print("📄 Loading page \(nextPage) for Latest Movies")
        
        Task {
            do {
                let newItems = try await TMDBService.shared.fetchLatestMovies(page: nextPage, language: language)
                ...
                await MainActor.run {
                    ...
                    isLoading = false
                    print("✅ Page \(nextPage) loaded - Total: \(items.count), hasMore: \(hasMore)")
                }
            } catch {
