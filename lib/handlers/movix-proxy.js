...
function buildMovieBoxCdnProxyUrl(cdnUrl, playReferer) {
  const referer = playReferer || 'https://api3.aoneroom.com/';
  return `${MOVIEBOX_CDN_WORKER}/?path=moviebox-cdn&url=${encodeURIComponent(cdnUrl)}&referer=${encodeURIComponent(referer)}`;
}
...
function mapExtractorStreamsToMovieBoxResponse(rawStreams) {
  return rawStreams.map((s) => {
    ...
    return {
      ...
      quality: s.quality,
      url: needsCdnProxy ? buildMovieBoxCdnProxyUrl(directUrl, referer) : directUrl,
      directUrl,
