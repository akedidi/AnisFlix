...
function buildMovieBoxCdnProxyUrl(cdnUrl, playReferer) {
  const referer = playReferer || 'https://fmoviesunblocked.net/';
...
const MOVIX_HEADERS = {
  ...
  'Referer': 'https://movix.bet/',
  'Origin': 'https://movix.bet',
};
  ...
  try {
    ...
    if (decodedPath === 'link') {
      ...
      try {
        ...
        if (type === 'movie') {
          // API pour films: https://api.movix.bet/api/links/movie/{id}
          apiUrl = `https://api.movix.bet/api/links/movie/${id}`;
        } else if (type === 'tv') {
          // API pour séries: https://api.movix.bet/api/links/tv/{id}?season={season}&episode={episode}
          if (!season || !episode) {
            return res.status(400).json({ error: 'Paramètres "season" et "episode" requis pour les séries' });
          }
          apiUrl = `https://api.movix.bet/api/links/tv/${id}?season=${season}&episode=${episode}`;
        } else {
    ...
    if (decodedPath === 'moviebox') {
      ...
      try {
        ...
        const { downloads, playReferer } = await movieBoxScraper.getStreams(
        ...
        const streams = downloads.map(download => {
          ...
          const proxyUrl = buildMovieBoxCdnProxyUrl(download.url, playReferer);
    ...
    let movixUrl;
    const baseMovixUrl = 'https://api.movix.bet';
