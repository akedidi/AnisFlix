  ...
  try {
    ...
    if (decodedPath === 'link') {
      ...
      try {
        ...
        if (type === 'movie') {
          // API pour films: https://api.movix.tax/api/links/movie/{id}
          apiUrl = `https://api.movix.tax/api/links/movie/${id}`;
        } else if (type === 'tv') {
          // API pour séries: https://api.movix.tax/api/links/tv/{id}?season={season}&episode={episode}
          if (!season || !episode) {
            return res.status(400).json({ error: 'Paramètres "season" et "episode" requis pour les séries' });
          }
          apiUrl = `https://api.movix.tax/api/links/tv/${id}?season=${season}&episode=${episode}`;
        } else {
    ...
    let movixUrl;
    const baseMovixUrl = 'https://api.movix.tax';
