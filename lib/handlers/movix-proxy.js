...
import { VidlinkScraper } from "../../api/_services/vidlink/index.js";
import { getMovieBoxStreams } from "../../api/_services/moviebox/index.js";
import { getAnimeKaiStreams } from "../../api/_services/animekai/index.js";
import { getYFlixStreams } from "../../api/_services/yflix/index.js";
...
// MovieBox CDN blocks Vercel egress IPs — stream via Cloudflare Worker instead
const MOVIEBOX_CDN_WORKER = process.env.MOVIEBOX_CDN_WORKER || 'https://anisflix.kedidi-anis.workers.dev';

function isHakunaymatataCdn(url) {
  try {
    return new URL(url).hostname.toLowerCase().endsWith('hakunaymatata.com');
  } catch {
    return false;
  }
}

function buildMovieBoxCdnProxyUrl(cdnUrl, playReferer, extra = {}) {
  const referer = playReferer || 'https://api3.aoneroom.com/';
  let qs = `path=moviebox-cdn&url=${encodeURIComponent(cdnUrl)}&referer=${encodeURIComponent(referer)}`;
  if (extra.cookie) qs += `&cookie=${encodeURIComponent(extra.cookie)}`;
  if (extra.userAgent) qs += `&ua=${encodeURIComponent(extra.userAgent)}`;
  return `${MOVIEBOX_CDN_WORKER}/?${qs}`;
}

function mapExtractorStreamsToMovieBoxResponse(rawStreams) {
  return rawStreams.map((s) => {
    const directUrl = s.decoded_url;
    const referer = s.headers?.Referer || 'https://api3.aoneroom.com';
    const fmt = String(s.format || '').toLowerCase();
    const streamType = fmt === 'mp4' ? 'mp4' : fmt === 'dash' ? 'dash' : 'hls';
    const needsCdnProxy = isHakunaymatataCdn(directUrl);

    return {
      name: `MovieBox (${s.language || 'VO'}) ${s.quality}`,
      quality: s.quality,
      url: needsCdnProxy
        ? buildMovieBoxCdnProxyUrl(directUrl, referer, {
          cookie: s.headers?.Cookie,
          userAgent: s.headers?.['User-Agent'],
        })
        : directUrl,
      directUrl,
      size: streamType === 'mp4' ? '' : 'Stream',
      type: streamType,
      codec: s.codec || null,
...
class VixSrcScraper {
  ...
  async extractStreamFromPage(url, contentType, contentId, seasonNum, episodeNum) {
    ...
    if (contentType === 'movie') {
      apiUrl = `${this.baseUrl}/api/movie/${contentId}`;
      pageReferer = `${this.baseUrl}/movie/${contentId}`;
      subtitleApiUrl = `https://sub.wyzie.ru/search?id=${contentId}`;
    } else {
      apiUrl = `${this.baseUrl}/api/tv/${contentId}/${seasonNum}/${episodeNum}`;
      pageReferer = `${this.baseUrl}/tv/${contentId}/${seasonNum}/${episodeNum}`;
      subtitleApiUrl = `https://sub.wyzie.ru/search?id=${contentId}&season=${seasonNum}&episode=${episodeNum}`;
    }

    try {
      // Step 1: Query Vixsrc Next.js API endpoint to retrieve embed path
      console.log(`[Vixsrc] Fetching API endpoint: ${apiUrl}`);
      ...
      const embedPath = apiResponse.data?.src;
      if (!embedPath) {
        console.warn(`[Vixsrc] No embed path returned for ${contentType} ${contentId}`);
        return null;
      }

      // Step 2: Fetch embed HTML page
      const embedUrl = embedPath.startsWith('http') ? embedPath : `${this.baseUrl}${embedPath}`;
      ...
      if (html.includes('window.masterPlaylist')) {
        // New format: window.masterPlaylist = { params: { 'token': '...', 'expires': '...' }, url: '...' }
        // Extract the entire masterPlaylist block
        const blockMatch = html.match(/window\.masterPlaylist\s*=\s*(\{[\s\S]*?\})\s*\n/);
        if (blockMatch) {
          ...
          if (urlMatch && tokenMatch && expiresMatch) {
            ...
            masterPlaylistUrl = `${baseUrl}${sep}token=${token}&expires=${expires}&h=1&lang=en`;
            console.log(`[Vixsrc] Extracted from masterPlaylist block: ${masterPlaylistUrl}`);
          }
        }

        // Fallback: try broader regex across whole page
        if (!masterPlaylistUrl) {
          const urlMatch = html.match(/url:\s*['"](https?:\/\/[^'"]*playlist[^'"]*)['"]/); 
          const tokenMatch = html.match(/['"]token['"]\s*:\s*['"]([^'"]+)['"]/);
          const expiresMatch = html.match(/['"]expires['"]\s*:\s*['"]([^'"]+)['"]/);
          if (urlMatch && tokenMatch && expiresMatch) {
            const sep = urlMatch[1].includes('?') ? '&' : '?';
            masterPlaylistUrl = `${urlMatch[1]}${sep}token=${tokenMatch[1]}&expires=${expiresMatch[1]}&h=1&lang=en`;
            console.log(`[Vixsrc] Extracted via fallback regex: ${masterPlaylistUrl}`);
          }
        }
      }

      // Try window.streams as secondary source
      if (!masterPlaylistUrl && html.includes('window.streams')) {
        const streamsMatch = html.match(/window\.streams\s*=\s*(\[[\s\S]*?\]);/);
        if (streamsMatch) {
          try {
            const streams = JSON.parse(streamsMatch[1].replace(/\\\/\//g, '/'));
            const activeStream = streams.find((s) => s.active) || streams[0];
            if (activeStream?.url) {
              const tokenMatch = html.match(/['"]token['"]\s*:\s*['"]([^'"]+)['"]/);
              const expiresMatch = html.match(/['"]expires['"]\s*:\s*['"]([^'"]+)['"]/);
              if (tokenMatch && expiresMatch) {
                const sep = activeStream.url.includes('?') ? '&' : '?';
                masterPlaylistUrl = `${activeStream.url}${sep}token=${tokenMatch[1]}&expires=${expiresMatch[1]}&h=1&lang=en`;
                console.log(`[Vixsrc] Extracted from window.streams: ${masterPlaylistUrl}`);
              } else {
                masterPlaylistUrl = activeStream.url;
              }
            }
          } catch (e) {
            console.warn('[Vixsrc] Failed to parse window.streams:', e.message);
          }
        }
      }
      ...
      console.error(`[Vixsrc] extraction error for ${contentId}: ${error.message}`);
      return null;
    }
  }
  ...
  async getStreams(tmdbId, mediaType = 'movie', seasonNum = null, episodeNum = null, appBaseUrl = null) {
    try {
      ...
      const baseUrl = appBaseUrl || process.env.PUBLIC_SITE_URL || 'https://anisflix.vercel.app';
      const proxyUrl = `${baseUrl}/api/vixsrc-proxy?url=${encodeURIComponent(masterPlaylistUrl)}`;
...
function unpackLuluvidPacker(html) {
  const start = html.indexOf('eval(function(p,a,c,k,e,d)');
  if (start === -1) return null;
  const slice = html.slice(start);
  const re = /eval\(function\(p,a,c,k,e,d\)\{while\(c--\)if\(k\[c\]\)p=p\.replace\(new RegExp\('\\\\b'\+c\.toString\(a\)\+'\\\\b','g'\),k\[c\]\);return p\}\('([\s\S]*?)',(\d+),(\d+),'([\s\S]*?)'\.split\('\|'\)\)\)/;
  ...
  const toBase = (n, radix) => {
    ...
    if (n === 0) return '0';
    let s = '';
    while (n > 0) { s = digits[n % radix] + s; n = Math.floor(n / radix); }
    return s;
  };
  while (c >= 0) {
    if (k[c]) {
      const token = toBase(c, a);
      p = p.replace(new RegExp(`\\b${token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'g'), k[c]);
    }
    c -= 1;
  }
  return p;
...
class LuluvidExtractor {
  ...
  async extract(url) {
    try {
      ...
      const html = response.data;
      let searchText = html;
      const unpacked = unpackLuluvidPacker(html);
      if (unpacked) {
      ...
      let m3u8 = null;
      let match = searchText.match(/sources:\s*\[\s*\{\s*file:\s*["']([^"']+\.m3u8[^"']*)["']/);
      if (!match) match = searchText.match(/file:\s*["']([^"']+\.m3u8[^"']*)["']/);
      if (!match) match = searchText.match(/(https?:\/\/[^\s"'<>]+\.m3u8[^\s"'<>]*)/);
  ...
  async getStreams(subjectId, season = 0, episode = 0) {
    ...
    if (cached) {
      if (Array.isArray(cached)) {
        return { downloads: cached, playReferer: 'https://fmoviesunblocked.net/' };
      }
      return cached;
    }
    ...
    try {
      ...
      const result = { downloads, playReferer: refererUrl };

      // Cache the result
      if (downloads.length > 0) {
        cacheSet(cacheKey, result, CACHE_TTL.STREAMS);
      }
      ...
      return { downloads: [], playReferer: 'https://fmoviesunblocked.net/' };
    }
  }
...
export default async function handler(req, res) {
  ...
  res.setHeader('X-Anisflix-Api-Rev', 'moviebox-cdn-worker-v1');
  ...
  try {
    ...
    if (decodedPath === 'cinepro-proxy') {
      ...
      try {
        ...
        if (decodedUrl.includes('luluvid.com') || decodedUrl.includes('lulustream.com')) {
    ...
    if (decodedPath === 'link') {
      ...
      try {
        ...
        if (type === 'movie') {
          // API pour films: https://api.movix.cloud/api/links/movie/{id}
          apiUrl = `https://api.movix.cloud/api/links/movie/${id}`;
        } else if (type === 'tv') {
          // API pour séries: https://api.movix.cloud/api/links/tv/{id}?season={season}&episode={episode}
          if (!season || !episode) {
            return res.status(400).json({ error: 'Paramètres "season" et "episode" requis pour les séries' });
          }
          apiUrl = `https://api.movix.cloud/api/links/tv/${id}?season=${season}&episode=${episode}`;
        } else {
    ...
    if (decodedPath === 'vixsrc') {
      ...
      try {
        ...
        const appBaseUrl = host ? `${protocol}://${host}` : null;
    ...
    if (decodedPath === 'moviebox') {
      ...
      try {
        ...
        const finalSeason = type === 'tv' ? (season ? parseInt(season) : 1) : 0;
        const finalEpisode = type === 'tv' ? (episode ? parseInt(episode) : 1) : 0;

        console.log(`📦 [MovieBox] Mobile extractor ${type} ${tmdbId} S${finalSeason}E${finalEpisode}`);
        const rawStreams = await getMovieBoxStreams(tmdbId, type, finalSeason, finalEpisode);

        if (!rawStreams.length) {
          return res.status(200).json({
            success: true,
            streams: [],
            message: 'No MovieBox streams available (MovieBox API anti-bot protection blocking server requests)',
          });
        }

        const streams = mapExtractorStreamsToMovieBoxResponse(rawStreams);
        console.log(`✅ [MovieBox] Returning ${streams.length} stream(s)`);
    ...
    // Legacy moviebox-stream URLs → relay via CF Worker (Vercel IPs are CDN-blocked)
    if (decodedPath === 'moviebox-stream') {
      try {
        const { url: videoUrl, referer: playReferer } = queryParams;

        if (!videoUrl) {
          return res.status(400).json({ error: 'Missing url parameter' });
        }

        if (!isHakunaymatataCdn(videoUrl)) {
          return res.status(400).json({ error: 'Invalid CDN URL' });
        }

        const workerUrl = buildMovieBoxCdnProxyUrl(videoUrl, playReferer);
        const range = req.headers.range;
        console.log(`📦 [MovieBox Stream] Relay via Worker: ${videoUrl.substring(0, 80)}...`);
        ...
        headersToForward.forEach((header) => {
          const value = workerRes.headers.get(header);
        ...
        console.error('📦 [MovieBox Stream] Worker relay error:', streamError.message);
        if (!res.headersSent) {
          return res.status(502).json({
    ...
    if (decodedPath === 'proxy/hls') {
      try {
        ...
        if (contentType && (contentType.includes('mpegurl') || contentType.includes('m3u8') || targetUrl.includes('.m3u8'))) {
          ...
          const protocol = req.headers['x-forwarded-proto'] || 'https';
          const host = req.headers.host;
          const proxyBaseUrl = `${protocol}://${host}/api/movix?path=proxy/hls&link=`;
    ...
    let movixUrl;
    const baseMovixUrl = 'https://api.movix.cloud';
