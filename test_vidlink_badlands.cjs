...
// Test function for Badlands
async function test() {
    console.log("Testing Vidlink extraction for Badlands (TMDB 1242898)...");
    const tmdbId = 1242898; // Badlands TMDB ID

    try {
        const streams = await getStreams(tmdbId, "movie");

        if (streams && streams.length > 0) {
            console.log("\n✅ Success! Found streams:");
            console.log(JSON.stringify(streams.map(s => ({
                name: s.name,
                url: s.url,
                quality: s.quality
            })), null, 2));
